"""Golden-file characterization test for the MarkTechPost RSS collector.

Pins the parser output against a saved RSS XML snapshot (3 entries, trimmed
from an actual A0 dump of https://www.marktechpost.com/feed/).
...
@pytest.mark.asyncio
async def test_marktechpost_collect_matches_golden(monkeypatch):
    xml = load_fixture("marktechpost_feed.xml")
    parsed_feed = feedparser.parse(xml)
    monkeypatch.setattr(feedparser, "parse", lambda _url: parsed_feed)

    expected = json.loads(load_fixture("marktechpost_feed.golden.json"))

    items = await MarkTechPostRssCollector().collect()
