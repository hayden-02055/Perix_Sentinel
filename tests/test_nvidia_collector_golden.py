"""Golden-file characterization test for the NVIDIA RSS collector.

Pins the parser output against a saved Atom XML snapshot (3 entries).
...
@pytest.mark.asyncio
async def test_nvidia_collect_matches_golden(monkeypatch):
    xml = load_fixture("nvidia_feed.xml")
    parsed_feed = feedparser.parse(xml)
    monkeypatch.setattr(feedparser, "parse", lambda _url: parsed_feed)

    expected = json.loads(load_fixture("nvidia_feed.golden.json"))

    items = await NvidiaRssCollector().collect()
